...
As part of my exchange semester at Stockholm University (Sweden), I completed several educational machine learning projects. In this project, we investigate two ways to classify a digit: the centroid algorithm and the PCA algorithm.
...
Since the 2\-norm distance between unknown digit and the average training digit 7 is the smallest, the classifier correctly predicts the digit as 7. Note also that average train digit 7 is actually T(8,:) because MATLAB indexing starts at 1.
...
4. Classifying the test set of digit by minimizing that distance
...
Despite relatively low accuracy for digits 5, 8 and 2, the centroid method seem to perform well on this dataset as accuracy rate is globally above 80%.


On the other hand, the PCA method seems to be more efficient than the previous one. With rates no lower than 88%, we can say that PCA is suitable for the analysis of this handwritten digit dataset. However, we can try to improve these rates by changing the length of the basis.
